...
{
    /// <summary>
    ///     Similar to a string or a StringBuilder, made for appending strings/ints/floats to each other so the result can be
    ///     displayed with a TextMeshPro.
    ///     <para />
    ///     After appending your content, call myTextMeshPro.SetText(myCharArray)
    ///     <para />
    ///     (Why?) Using System.String to represent dynamic values will always involve GC allocations.  StringBuilder addresses
    ///     this, but appending numbers will involve GC allocations because it calls number.ToString(). The goal of CharArray
    ///     is to hide these pain-points and allow GC-free textbox updating by default, as long as you provide an appropriate
    ///     initial capacity.
    /// </summary>
    [Serializable]
    public class CharArray : FastArray<char>
    {
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public CharArray(string fromString, int capacity = -1) :
            base(capacity == -1 ? fromString.Length * 2 : capacity) =>
            Append(fromString);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ApplyToTextMeshPro(TMP_Text target) => target.SetText(this);
        ...
        /// <param name="minLengthPadLeft">
        ///     The minimum digit-count - extra
        ///     digits will be left-padded.  Append(100, 6) will append "000100"
        /// </param>
        public void Append(int value, int minLengthPadLeft = 0)
        {
            ...
            for (var i = digitCount; i < minLengthPadLeft; i++)
            {
                Add('0');
                digitCount++;
            }

            var numberEnd = count - 1;
            for (var i = 0; i < digitCount / 2; i++)
            {
                var index1 = numberStart + i;
                var index2 = numberEnd - i;
        ...
        /// <param name="digitsAfterDecimal">
        ///     The number of digits after the decimal place.
        ///     (Append(2f, 3) is equivalent to myString+="2.000")
        ///     <param>
        public void Append(float value, int digitsAfterDecimal = 1)
        {
            ...
            if (digitsAfterDecimal > 0)
            {
                ...
                long scale = 1;
                for (var i = 0; i < digitsAfterDecimal; i++) scale *= 10;

                var floatDecimalDigits = (double) (value - (int) value) * scale;
                var decimalDigits = (long) floatDecimalDigits;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Append(char c) => Add(c);
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public override string ToString() => new string(array);
    }
